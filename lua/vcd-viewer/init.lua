...
function M.parse(lines)
	...
	-- Skip header until we find $var
	while i <= #lines and not lines[i]:match("^%s*$var") do
		i = i + 1
	end

	-- Parse declarations
	while i <= #lines and lines[i]:match("^%s*$var") do
	...
	-- Skip until definitions end or timestamp start
	while i <= #lines and not (lines[i]:match("^#%d+") or lines[i]:match("^%d[!]")) do
		i = i + 1
	end
